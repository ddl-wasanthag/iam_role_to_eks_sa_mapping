...
def map_org_to_iam_role() -> object:
    ...
    if not ('domino_org' in payload and 'iam_role' in payload):
        return Response(
            str('Pay load must contain attributes domino_org and iam_role'),
            404)
    else:
        ...
        if not domino_org or not iam_role:
            return Response(
                str('Pay load must contain a non-empty domino org and a not-empty iam_role'),
                404)
        old_gcp_sa, new_gcp_sa = aws_utils.update_orgs_iam_roles_mapping(domino_org,iam_role,PLATFORM_NS)
        return Response(
            str(f'Domino Org {domino_org} mapping updated from IAM ROLE {old_gcp_sa} to {new_gcp_sa}'),
            200)
...
def map_iam_roles_to_aws_sa() -> list:
    ...
    for role in aws_resource_roles_by_name.keys():
        ...
        aws_config_file = aws_config_file + f"role_arn={aws_resource_roles_by_name[role]}\n"
